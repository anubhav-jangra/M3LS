...
class bbcSpider(scrapy.Spider):
   ...
   # start_urls = ['https://www.bbc.com/russian/features-58837910']
    ...
    def parse(self, response):
        ...
        try:
            domain = domain_split(url_main.split('russian/')[1])  # generalize this (Tentative: Different language scrapers will only have this field different and
        except:
            domain = "NA"
...
"hash": url_hash,                 'type': 'summary',
        ...
        for i in response.css('section[data-e2e="related-content-heading"] li'):
          ...
          #  rel_head = i.css('h3 ::text').getall()[-1]
            ...
            rel_tuple = ({'url': rel_url}, {'date', rel_date}, {'hash', rel_hash})
...
#        ex_lnk = set(pd.read_csv('~/yash/russian/bbcrussian/bbcrussian/spiders/extralinks.csv', header = None)[0])
#        extra_links.difference_update(ex_lnk)
      # print (rest_links, "\n\n\n\n\\n\n\n=====================\n\\n\n\n\n\n\n\n===========")
 #       pd.DataFrame(np.asarray(list(extra_links))).to_csv('~/yash/russian/bbcrussian/bbcrussian/spiders/extralinks.csv', header = False, mode = 'a', index = False)

        # save last visited link, if there is a crash
        pd.DataFrame(np.asarray([url_main])).to_csv('lastlink.csv', index = False, mode = 'w', header= False)

...
class bbcSpider(scrapy.Spider):
    ...
    data = pd.read_csv('extractedlinks.csv', header = None)
    valid_ind = data[1].str.contains('bbc') & data[1].str.startswith('http')
    urls = data[1][valid_ind == True]
    ...
    # start_urls = ['https://www.bbc.com/hindi/international-57938439']
    ...
    def parse(self, response):
        ...
        div = response.css('main>div')[:-1] # [-1] is to remove the last div which contains share and social media links
        ...
        try:
            domain = domain_split(url_main.split('hindi/')[1])  # generalize this (Tentative: Different language scrapers will only have this field different and
        except:
            domain = "NA"
        # end div with social links or divs with recommended links different) also for old urls
        if 'media' in url_main:
            # Media articles are the ones with only video and short text, no summary
            pd.DataFrame(np.asarray([url_main])).to_csv('medialinksf2.csv', index = False, mode = 'a', header = False)
            return

        if 'live'  in url_main:
            # handling for live here (ITEM PIPELINE)
            pd.DataFrame(np.asarray([url_main])).to_csv('livelinksf2.csv', index = False, mode = 'a', header = False)
            return

        if 'podcast' in url_main:
            pd.DataFrame(np.asarray([url_main])).to_csv('podcastlinksf2.csv', index = False, mode = 'a', header = False)
            return
        ...
        for i in div:
            ...
            # If div has some other links, we store them too
                ...
                extra_links.extend(i.css('a::attr(href)').getall())
            ...
            # Check for date
                ...
                yield{"hash": url_hash, 'type': 'date', 'date': i.css('time').attrib['datetime']}

            # Check for image
            elif i.css('figure'):
                url = i.css('img').attrib['src']
...
"hash": url_hash,                 'type': 'summary',
            ...
            # short subheadings
                ...
                c = ''.join(r)
                # Hindi Specific code
                if c == 'ये भी पढ़ें' or c == 'यह भी पढ़ें' or "ऐप के लिए" in c or len(c) < 15:
                    pass
                else:
                    subhead += 1
            ...
            # Else paragraph normal
                ...
                if (i.css('a')):
                    if (i.css('p a::text').get()) != None and (i.css('p::text').get()) != None:
                        x = i.css('p a::text').get()
                        y =  i.css('p::text').get()
                        yield{"hash": url_hash, 'type': 'para', 'text' : x  + y + "\n", 'subhead' : subhead}

                elif (i.css('p::text').get()) != None:
                    yield {"hash": url_hash, 'type': 'para', 'text' :  i.css('p::text').get(), 'subhead': subhead}
        ...
        for i in response.css('section[data-e2e="related-content-heading"] li'):
            ...
            rel_head = i.css('h3 ::text').getall()[-1]
            try:
                rel_date = i.css('time').attrib['datetime']
            except:
                rel_date = 'NA'
            rel_hash = hashit(rel_url)
            rel_tuple = ({'url': rel_url}, {'head', rel_head}, {'date', rel_date}, {'hash', rel_hash})
...
#        pd.DataFrame(np.asarray(extra_links)).to_csv('extractedlinks2.csv', index = False, mode = 'a', header = False)

        # save last visited link, if there is a crash
        pd.DataFrame(np.asarray([url_main])).to_csv('lastlinkf2.csv', index = False, mode = 'w', header= False)

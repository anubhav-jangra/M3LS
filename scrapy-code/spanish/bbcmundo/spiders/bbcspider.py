...
class bbcSpider(scrapy.Spider):
  ...
  #  start_urls = ['https://www.bbc.com/mundo/noticias-58828993']
    ...
    def parse(self, response):
        ...
        try:
            domain = domain_split(url_main.split('mundo/')[1])  # generalize this (Tentative: Different language scrapers will only have this field different and
        except:
            domain = "NA"
        # end div with social links or divs with recommended links different) also for old urls
        if 'media' in url_main or '/av' in url_main:
            # Media articles are the ones with only video and short text, no summar
        ...
        for i in div:
            ...
            # Check for date
                ...
                yield{"hash": url_hash, 'type': 'date', 'date': i.css('time::attr(datetime)').get()}
...
"hash": url_hash,                 'type': 'summary',
        ...
        for i in response.css('section[data-e2e="related-content-heading"] li'):
            ...
            rel_tuple = ({'url': rel_url}, {'head', rel_head}, {'date', rel_date}, {'hash', 
rel_hash})
   ...
   #     ex_lnk = set(pd.read_csv('~/yash/spanish/bbcmundo/bbcmundo/spiders/extralinks.csv', header = None)[0])
   #     extra_links.difference_update(ex_lnk)
   #     l_lnk = set(pd.read_csv('~/yash/spanish/bbcmundo/lastlink.csv', header = None)[0])
   #     extra_links.difference_update(l_lnk)
      # print (rest_links, "\n\n\n\n\\n\n\n=====================\n\\n\n\n\n\n\n\n===========")
   #     pd.DataFrame(np.asarray(list(extra_links))).to_csv('~/yash/spanish/bbcmundo/bbcmundo/spiders/extralinks.csv', header = False, mode = 'a', index = False)

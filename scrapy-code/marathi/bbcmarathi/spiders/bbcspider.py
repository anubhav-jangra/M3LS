...
class bbcSpider(scrapy.Spider):
    ...
    #start_urls = ['https://bbc.in/3ucM0qj']
    ...
    def parse(self, response):
        ...
        try:
            domain = domain_split(url_main.split('nepali/')[1])  # generalize this (Tentative: Different language scrapers will only have this field different and
        except:
            domain = "NA"
        ...
        for i in div:
            ...
            # Check for image
                ...
                #    print (url, "HERE\n\n\\n\n\n\\n\n\n\n\\n\n\n\\n\\n\n\n\\n\n=======================\n\n\n\\n\n\n\n", hashit(url))
                    ...
                    if not isinstance(url , str) and not isinstance(img_nm, str):
                        print ('url', url)
                        print ('img_nm', img_nm)
...
"hash": url_hash,                 'type': 'summary',
                'summary': ''.join(i.css('p>b *::text').getall()),
                }
            # short subheadings
         #   elif i.css('p>b') and flag == True:
          ...
          #      c = ''.join(r)
            ...
            # Else paragraph normal
                ...
                if (i.css('a')):
                    if "ஊடகஙகளல பபச தமழ" in i.css('::text').getall()  or 'பற சயதகள' in i.css('::text').getall(): # modify this for punjabi
                        continue
...
rel_hash})
        ...
        ex_lnk = set(pd.read_csv('~/yash/bbcmarathi/bbcmarathi/bbcmarathi/spiders/extralinks2.csv', header = None)[0])
        extra_links.difference_update(ex_lnk)
        l_lnk = set(pd.read_csv('~/yash/bbcmarathi/bbcmarathi/lastlink.csv', header = None)[0])
        extra_links.difference_update(l_lnk)
      # print (rest_links, "\n\n\n\n\\n\n\n=====================\n\\n\n\n\n\n\n\n===========")
        pd.DataFrame(np.asarray(list(extra_links))).to_csv('~/yash/bbcmarathi/bbcmarathi/bbcmarathi/spiders/extralinks2.csv', header = False, mode = 'a', index = False)

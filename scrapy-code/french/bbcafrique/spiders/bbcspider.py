...
class bbcSpider(scrapy.Spider):
   ...
   # start_urls = ['https://www.bbc.com/afrique/region-58818895']
    ...
    def parse(self, response):
        ...
        try:
            domain = domain_split(url_main.split('afrique/')[1])  # generalize this (Tentative: Different language scrapers will only have this field different and
        except:
            domain = "NA"
...
rel_hash})
 ...
 #       extra_links = set(extra_links)
  #      ex_lnk = set(pd.read_csv('~/yash/french/bbcafrique/bbcafrique/spiders/extralinks.csv', header = None)[0])
   #     extra_links.difference_update(ex_lnk)
    #    l_lnk = set(pd.read_csv('~/yash/french/bbcafrique/lastlink.csv', header = None)[0])
    #    extra_links.difference_update(l_lnk)
      # print (rest_links, "\n\n\n\n\\n\n\n=====================\n\\n\n\n\n\n\n\n===========")
     #   pd.DataFrame(np.asarray(list(extra_links))).to_csv('~/yash/french/bbcafrique/bbcafrique/spiders/extralinks.csv', header = False, mode = 'a', index = False)

...
class bbcSpider(scrapy.Spider):
    ...
    data = pd.read_csv('extralinksold.csv', header = None)[0]
 ...
 #   start_urls = ['https://www.bbc.com/japanese/37946636']
    ...
    def parse(self, response):
        ...
        if 'japan' not in url_main:
            return
    
        if 'http' not in url_main[:4]:
...
#        pd.DataFrame(np.asarray([url_main])).to_csv('~/yash/bbcjapanese/lastlink.csv', index = False, mode = 'a', header= False)

#        extra_links = set(extra_links)
#        ex_lnk = set(pd.read_csv('~/yash/bbcjapanese/bbcjapanese/spiders/extralinks.csv', header = None)[0])
#        extra_links.difference_update(ex_lnk)
#        l_lnk = set(pd.read_csv('~/yash/bbcjapanese/lastlink.csv', header = None)[0])
#        extra_links.difference_update(l_lnk)
      # print (rest_links, "\n\n\n\n\\n\n\n=====================\n\\n\n\n\n\n\n\n===========")
#        pd.DataFrame(np.asarray(list(extra_links))).to_csv('~/yash/bbcjapanese/bbcjapanese/spiders/extralinks.csv', header = False, mode = 'a', index = False)
